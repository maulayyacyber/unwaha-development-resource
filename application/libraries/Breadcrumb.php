...
class Breadcrumb {
	 ...
	 */
	private $_divider 		= '';
	private $_tag_open 		= '<ul class="page-breadcrumb breadcrumb">';
	 ...
	 * @return	void
	 */		
	function append_crumb($class_icon, $title, $href, $class_arrow)
	{
		// no title or href provided
		if (!$title OR !$href) return;
		
		// add to end
		$this->breadcrumbs[] = array('class_icon' => $class_icon, 'title' => $title, 'href' => $href, 'class_arrow' => $class_arrow);
	}
	 ...
	 * @return	void
	 */		
	function prepend_crumb($class_icon, $title, $href, $class_arrow)
	{
		// no title or href provided
		if (!$title OR !$href) return;
		
		// add to start
		array_unshift($this->breadcrumbs, array('class_icon' => $class_icon, 'title' => $title, 'href' => $href, 'class_arrow' => $class_arrow));
	}
		...
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				...
				if (end(array_keys($this->breadcrumbs)) == $key) {
					$output .= ' <li><i class="'. $crumb['class_icon'] .'"></i><a href="' . $crumb['href'] . '">' . $crumb['title'] . '</a><i class="'. $crumb['class_arrow'] .'"></i></li>';
